...
class TraversalTree:
    ...
    def generate_children(self):
        ...
        for node in self.path[-1].neighbours:
            if not node.start and not (node.small and node in self.path):
                children.append(TraversalTree(self.path + [node]))
...
class Network:
    ...
    def build_traversal_tree(self):
        return TraversalTree([self.start])
